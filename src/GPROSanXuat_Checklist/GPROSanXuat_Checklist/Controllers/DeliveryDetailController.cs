...
using GPROSanXuat_Checklist.Helper;
...
namespace GPROSanXuat_Checklist.Controllers
{
    ...
    {
        [HttpPost]
        public JsonResult Gets(int Id, int jtStartIndex = 0, int jtPageSize = 0, string jtSorting = "")
        {
            try
            {
                var objs = BLLDeliveryDetail.Instance.GetList(AppGlobal.ConnectionstringSanXuatChecklist, Id, jtStartIndex, jtPageSize, jtSorting);
                
                JsonDataResult.Records = DeliveryDetailsMaper.Instance.MapInfoFromGPROCommon(objs);
        ...
        [HttpPost]
        public JsonResult Save( DeliveryDetailModel obj)
        {
            ResponseBase rs;
            try
            {
                ...
                rs = BLLDeliveryDetail.Instance.CreateOrUpdate(AppGlobal.ConnectionstringSanXuatChecklist, obj);
                if (!rs.IsSuccess)
                {
        ...
        public JsonResult Delete(int Id)
        {
            ...
            try
            {
                result = BLLDeliveryDetail.Instance.Delete(AppGlobal.ConnectionstringSanXuatChecklist, Id, UserContext.UserID);
                if (!result.IsSuccess)
                {
                    JsonDataResult.Result = "ERROR";
        ...
        public void ExportToExcel(int deliveryId)
        {
            try
            {
                #region get data
                var objModel = BLLDeliveryDetail.Instance.GetDeliveryDetails(AppGlobal.ConnectionstringSanXuatChecklist, deliveryId);
                objModel = DeliveryDetailsMaper.Instance.MapInfoFromGPROCommon(objModel);
                #endregion
                var excelPackage = new ExcelPackage();
                excelPackage.Workbook.Properties.Author = "GPRO";
                excelPackage.Workbook.Properties.Title = "Phiếu Xuất Kho";
                ...
                if (objModel != null && objModel.Count > 0)
                {
                    ...
                    sheet.Cells[7, 1].Value = "PHIẾU XUẤT KHO";
                    ...
                    sheet.Cells[9, 1].Value = "Số : " + objModel[0].DeliveryCode;
                    ...
                    sheet.Cells[13, 1].Value = "Họ và tên người nhận hàng : " + objModel[0].ReceiverName;
                    sheet.Cells[13, 1, 13, 4].Merge = true;

                    sheet.Cells[13, 6].Value = "Địa chỉ (bộ phận) : ";
                    sheet.Cells[13, 6, 13, 9].Merge = true;

                    sheet.Cells[14, 1].Value = "Lý do xuất kho : ";
                    sheet.Cells[14, 1, 14, 4].Merge = true;
                    ...
                    //        str += objModel[i].LotName + ", ";
                    //}
                    sheet.Cells[15, 1].Value = "Xuất tại kho (ngăn lô) : " + objModel[0].WareHouseName;
                    ...
                    sheet.Cells[18, 7].Value = "Thực xuất";
                    ...
                    ExcelRichText boldText, normalText;
                    //sheet.Cells[rowIndex, 1].Value = "Tổng số tiền (viết bằng chữ) : " + SoTienBangChu.ChuyenSoTienSangChu(sumofmoney);
                    sheet.Cells[rowIndex, 1].RichText.Add("Tổng số tiền (viết bằng chữ) : ");
                    boldText = sheet.Cells[rowIndex, 1].RichText.Add(SoTienBangChu.ChuyenSoTienSangChu(sumofmoney, objModel[0].MoneyTypeId, objModel[0].MoneyTypeName));
                    boldText.Bold = true;
                    ...
                    //sheet.Cells[rowIndex, 6].Value = "Kế toán trưởng\r\n(Hoặc bộ phận có nhu cầu nhập\r\n(Ký, họ tên)";
                    ...
                    //sheet.Cells[rowIndex, 1].Value = "Người lập phiếu\r\n(Ký, họ tên)";
                    boldText = sheet.Cells[rowIndex, 1].RichText.Add("Người lập phiếu\r\n");
                    boldText.Bold = true;
                    normalText = sheet.Cells[rowIndex, 1].RichText.Add("(Ký, họ tên)");
                    normalText.Bold = false;
                    ...
                    //sheet.Cells[rowIndex, 3].Value = "Người nhận hàng\r\n(Ký, họ tên)";
                    boldText = sheet.Cells[rowIndex, 3].RichText.Add("Người nhận hàng\r\n");
                    boldText.Bold = true;
                    normalText = sheet.Cells[rowIndex, 3].RichText.Add("(Ký, họ tên)");
                    normalText.Bold = false;
                    ...
                    //sheet.Cells[rowIndex, 4].Value = "Thủ kho\r\n(Ký, họ tên)";
                    boldText = sheet.Cells[rowIndex, 4].RichText.Add("Thủ kho\r\n");
                    boldText.Bold = true;
                    normalText = sheet.Cells[rowIndex, 4].RichText.Add("(Ký, họ tên)");
                    normalText.Bold = false;
                    ...
                    //sheet.Cells[rowIndex, 8].Value = "Giám đốc\r\n(Ký, họ tên)";
                    boldText = sheet.Cells[rowIndex, 8].RichText.Add("Giám đốc\r\n");
                    boldText.Bold = true;
                    normalText = sheet.Cells[rowIndex, 8].RichText.Add("(Ký, họ tên)");
                    normalText.Bold = false;
                ...
                string fileName;
                if (objModel != null && objModel.Count > 0)
                    fileName = "PhieuXuatKho-"+objModel[0].DeliveryName + "(" + objModel[0].DeliveryCode + ")-" + dateNow.ToString("yyMMddhhmmss") + ".xlsx";
                else
                    fileName = "PhieuXuatKho-Empty File-" + dateNow.ToString("yyMMddhhmmss") + ".xlsx";
